...
var get_value = function(d) { console.log("d in gtevale", d); return d.value; }
var mins = 0; maxs = 0;
...
function runAQueryOn(error, indicators, data) {
    ...
    //http://api.worldbank.org/countries/all/indicators/SP.POP.TOTL?format=json&date=2000:2001
    //http://api.worldbank.org/countries/all/indicators/SP.POP.TOTL?format=jsonP&prefix=Getdata&date=2000:2001
    //http://api.worldbank.org/countries/all/indicators/SH.TBS.DTEC.ZS?format=jsonP&prefix=Getdata&date=2012
    $.ajax({
//        url: "http://api.worldbank.org/countries/all/indicators/" + sel_metric + "?format=jsonP&prefix=Getdata&date=" + sel_year , //do something here
        ...
        success: function (data, status){
            console.log("Got data for chloro", data)
...
var initVis = function(error, indicators, data){
    ...
    var test = color.range().map(function(d, i) {
      return {
    ...
    console.log("color.range:", color.domain()[0])
    g.selectAll("rect")
...
g.call(xAxis).append("text")
    ...
    var options2 = dropDown2.selectAll("option")
           ...
           //.attr("selected" , function (d,i) { if i == 2 return 'selected'; })
...
var createDetailVis = function(){
  ...
  console.log("createDetailVis called")
